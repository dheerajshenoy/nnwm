...
#ifdef HAVE_SCENEFX
static void
set_corner_radii_recursive(struct wlr_scene_tree *tree,
                           struct fx_corner_radii radii)
{
    ...
    wl_list_for_each(child, &tree->children, link)
    {
        if (child->type == WLR_SCENE_NODE_BUFFER)
            wlr_scene_buffer_set_corner_radii(wlr_scene_buffer_from_node(child),
                                              radii);
...
update_borders(nnwm_toplevel *toplevel, int width, int height, int bw)
{
    ...
    bool no_titlebar
        = tabbed_tiled || toplevel->fullscreen || toplevel->fake_fullscreen;
    ...
    /* window surface is pushed down by the titlebar (omitted in
     * tabbed/fullscreen) */
    wlr_scene_node_set_position(&toplevel->scene_surface->node, bw,
                                bw + (no_titlebar ? 0 : th));
...
tl_set_geometry(nnwm_toplevel *tl, int x, int y, int w, int h, int bw)
{
    ...
    if (tl->is_xwayland && tl->xwayland_surface && changed)
    {
        ...
        int eff_th
            = (no_tb3 || cfg->titlebar.height <= 0) ? 0 : cfg->titlebar.height;
        int cx = x + bw;
        int cy = y + bw + eff_th;
        int cw = w - 2 * bw;
        ...
        if (ch < 1)
            ch = 1;
        nnwm_xw_configure(tl->xwayland_surface, (int16_t)cx, (int16_t)cy,
    ...
    if (do_anim && changed)
    {
        if (first)
        {
            ...
            nnwm_open_style open_style
                = tl->rule_no_anim == 1 ? nnwm_open_style::NONE
    ...
    if (tl->is_xwayland && tl->xwayland_surface)
    {
        ...
        int eff_bw        = bw;
        int eff_th        = (no_titlebar2 || cfg2->titlebar.height <= 0)
                                ? 0
                                : cfg2->titlebar.height;
        int cx            = x + eff_bw;
...
tl_start_border_color(nnwm_toplevel *tl, const float to[4])
{
    ...
    for (int i = 0; i < 4; i++)
        tl->bcol_to[i] = to[i];
    tl->bcol_duration_ms
        = eff_duration(cfg, cfg->fx.animation.focus_duration_ms);
...
animate_step_one(nnwm_server * /*server*/, nnwm_toplevel *tl, double now)
{
    ...
    if (tl->geo_anim)
    {
        ...
        {
            bool on_active
                = (tl->workspace == tl->output->active_workspace || tl->sticky);
            wlr_box ob;
...
apply_fx_decorations(nnwm_toplevel *toplevel)
{
    ...
    if (eff_blur)
    {
        ...
        if (!toplevel->fx_blur)
        {
            ...
            if (toplevel->is_xwayland)
            {
                w = (toplevel->cur_w > 0)
                        ? toplevel->cur_w
                        : nnwm_xw_width(toplevel->xwayland_surface) + 2 * bw;
                h = (toplevel->cur_h > 0)
                        ? toplevel->cur_h
                        : nnwm_xw_height(toplevel->xwayland_surface) + 2 * bw
                              + th;
            }
            else
    #endif
    ...
    if (cfg->fx.shadow_enabled && !toplevel->fx_shadow)
    {
        ...
        if (toplevel->is_xwayland)
        {
            ...
            h = (toplevel->cur_h > 0)
                    ? toplevel->cur_h
                    : nnwm_xw_height(toplevel->xwayland_surface) + 2 * bw + th;
        }
        else
    #endif
        {
        ...
        toplevel->fx_shadow = wlr_scene_shadow_create(
            toplevel->scene_tree, w, h, r, cfg->fx.shadow_blur_sigma,
            cfg->fx.shadow_color);
        wlr_scene_node_set_position(&toplevel->fx_shadow->node,
                                    (int)cfg->fx.shadow_offset_x,
                                    (int)cfg->fx.shadow_offset_y);
    ...
    else if (toplevel->fx_shadow)
    {
        ...
        else
        {
            ...
            wlr_scene_shadow_set_color(toplevel->fx_shadow,
                                       cfg->fx.shadow_color);
            wlr_scene_node_set_position(&toplevel->fx_shadow->node,
                                        (int)cfg->fx.shadow_offset_x,
                                        (int)cfg->fx.shadow_offset_y);
...
static void
draw_tab_text(cairo_t *cr, nnwm_config *cfg, const char *title, double tx,
              double ty, double tw, double th, const float *tc)
{
...
render_tab_bar(nnwm_server *server, nnwm_output *out, int width, int height)
{
    ...
    bool vertical
        = (pos == nnwm_tab_position::LEFT || pos == nnwm_tab_position::RIGHT);
    ...
    wl_list_for_each(tl, &server->toplevels, link)
    {
        ...
        if (!vertical)
        {
            ...
            if (style == nnwm_tab_style::NORMAL)
                draw_tab_text(cr, cfg, tl_title(tl), x, 0, w, height, tc);
        }
        else
        {
            ...
            if (style == nnwm_tab_style::NORMAL)
            {
                ...
                draw_tab_text(cr, cfg, tl_title(tl), -h / 2.0, -width / 2.0, h,
                              width, tc);
                cairo_restore(cr);
            }
        }
...
{
    if (!out
        || out->layout_mode[out->active_workspace] != nnwm_layout_mode::TABBED)
        return;
...
{
    if (!out->overview_buf || !out->overview_labels)
        return;
...
{
    if (!out->overview || !out->overview_buf || !out->overview_labels)
        return;
    ...
    {
        double now = anim_now();
        float t    = anim_t(out->ov_anim_t0, now, out->ov_anim_duration_ms,
                            nnwm_easing::OUT);
...
render_overview_buffers(nnwm_server *server, nnwm_output *out)
{
    ...
    {
        gpu_buf
            = wlr_allocator_create_buffer(server->allocator, buf_w, buf_h, fmt);
        if (gpu_buf)
            pass = wlr_renderer_begin_buffer_pass(server->renderer, gpu_buf,
                                                  nullptr);
    }
    ...
    if (pass)
    {
        ...
        wlr_texture *bg_tex = nullptr;
        {
            ...
            {
                if (ls->wlr_layer_surface->output != out->wlr_output)
                    continue;
                if (ls->wlr_layer_surface->current.layer
                    != ZWLR_LAYER_SHELL_V1_LAYER_BACKGROUND)
                    continue;
                bg_tex
                    = wlr_surface_get_texture(ls->wlr_layer_surface->surface);
                if (bg_tex)
                    break;
            }
        }
        ...
        for (int ws = 0; ws < num_ws; ws++)
        {
            ...
            pixman_region32_init_rect(&clip, psx, psy, (uint32_t)psw,
                                      (uint32_t)psh);
            ...
            wl_list_for_each(tl, &server->toplevels, link)
            {
                ...
                wlr_surface_for_each_surface(tl_wlr_surface(tl),
                                             ov_surface_iter, &d);
            }
        ...
        wlr_scene_node_set_position(&out->overview_buf->node, out_box.x,
                                    out_box.y);
                       ...
                       bool gpu_ok)
{
    ...
    for (int ws = 0; ws < num_ws; ws++)
    {
        ...
        if (!pass)
        {
            ...
            wl_list_for_each(tl, &server->toplevels, link)
            {
                ...
                wlr_box lb;
                if (!tl_layout_box(tl, bw, eff_th_fb, &lb))
                    continue;
                any          = true;
        ...
        else
        {
            ...
            wl_list_for_each(tl, &server->toplevels, link)
            {
                ...
                wlr_box lb;
                if (!tl_layout_box(tl, bw, eff_th_ck, &lb))
                    continue;
                any = true;
    ...
    wlr_scene_node_set_position(&out->overview_labels->node, out_box.x,
                                out_box.y);
...
{
    if (!out->overview || !out->overview_labels)
        return;
     ...
     * on the active workspace would stay hidden without this sweep). */
    {
        ...
        wl_list_for_each(t, &server->toplevels, link)
        {
            ...
            wlr_scene_node_set_enabled(
                ...
                t->sticky
                    || (t->output
                        && t->output->active_workspace == t->workspace));
        }
    }
...
scratch_first(nnwm_server *server)
{
    nnwm_toplevel *t;
    wl_list_for_each(t, &server->toplevels,
                     link) if (t->in_scratchpad
                               && t->scratchpad_name.empty()) return t;
    return nullptr;
}
...
scratch_last(nnwm_server *server)
{
    nnwm_toplevel *t, *last = nullptr;
    wl_list_for_each(t, &server->toplevels,
                     link) if (t->in_scratchpad && t->scratchpad_name.empty())
        last = t;
    return last;
}
...
get_or_create_named_scratchpad(nnwm_server *server, const std::string &name)
{
    ...
    nsp->dim_rect = wlr_scene_rect_create(server->scene_named_scratchpads, 0, 0,
                                          dim_color);
...
named_scratch_first(nnwm_server *server, const std::string &name)
{
    ...
    wl_list_for_each(t, &server->toplevels,
                     link) if (t->in_scratchpad
                               && t->scratchpad_name == name) return t;
    return nullptr;
}
...
named_scratch_count(nnwm_server *server, const std::string &name)
{
    ...
    nnwm_toplevel *t;
    wl_list_for_each(t, &server->toplevels,
                     link) if (t->in_scratchpad && t->scratchpad_name == name
                               && !t->floating) n++;
    return n;
}
...
arrange_named_scratchpad(nnwm_server *server, nnwm_named_scratchpad *nsp)
{
    ...
    if (nsp->layout == nnwm_layout_mode::HTILE)
    {
        if (n == 1)
        {
            wl_list_for_each(tl, &server->toplevels, link)
            {
                if (!tl->in_scratchpad || tl->scratchpad_name != nsp->name
                    || tl->floating)
                    continue;
...
arrange_windows_impl(nnwm_server *server, nnwm_output *out)
{
    ...
    if (out->layout_mode[ws] == nnwm_layout_mode::FLOAT)
    {
        ...
        wl_list_for_each(tl, &server->toplevels, link)
        {
            ...
            if (tl->workspace != ws && !tl->sticky)
                continue;
            if (tl->in_scratchpad || tl->fullscreen || tl->fake_fullscreen)
                continue;
            if (!tl->floating)
            {
                tl->floating = true;
    ...
    if (out->layout_mode[ws] == nnwm_layout_mode::TABBED)
    {
        ...
        wl_list_for_each(tl, &server->toplevels, link)
        {
            ...
            tl_xdg_set_tiled(tl, WLR_EDGE_TOP | WLR_EDGE_BOTTOM | WLR_EDGE_LEFT
                                     | WLR_EDGE_RIGHT);
            tl_xdg_set_size(tl, cw - 2 * bw, ch - 2 * bw);
        ...
        else if (n > 0)
        {
...
#ifdef HAVE_SCENEFX
            {
                ...
                {
                    case nnwm_tab_position::BOTTOM:
                        tab_cr = corner_radii_bottom(tab_r);
                        break;
                    case nnwm_tab_position::LEFT:
                        tab_cr = corner_radii_left(tab_r);
                        break;
                    case nnwm_tab_position::RIGHT:
                        tab_cr = corner_radii_right(tab_r);
                        break;
                    default:
                        tab_cr = corner_radii_top(tab_r);
                        break;
                }
    ...
    if (out->layout_mode[ws] == nnwm_layout_mode::HSCROLL)
    {
        ...
        wl_list_for_each(tl, &server->toplevels, link)
        {
            ...
            tl_xdg_set_tiled(tl, WLR_EDGE_TOP | WLR_EDGE_BOTTOM | WLR_EDGE_LEFT
                                     | WLR_EDGE_RIGHT);
            tl_xdg_set_size(tl, col_w - 2 * bw, col_h - 2 * bw - th);
    ...
    if (out->layout_mode[ws] == nnwm_layout_mode::VSCROLL)
    {
        ...
        wl_list_for_each(tl, &server->toplevels, link)
        {
            ...
            tl_xdg_set_tiled(tl, WLR_EDGE_TOP | WLR_EDGE_BOTTOM | WLR_EDGE_LEFT
                                     | WLR_EDGE_RIGHT);
            tl_xdg_set_size(tl, row_w - 2 * bw, row_h - 2 * bw - th);
    ...
    wl_list_for_each(tl, &server->toplevels, link)
    {
        if (tl->output != out || (tl->workspace != ws && !tl->sticky))
            continue;
        if (!tl->maximize || tl->floating || tl->fullscreen
            || tl->fake_fullscreen)
            continue;
        tl_xdg_set_tiled(tl, WLR_EDGE_TOP | WLR_EDGE_BOTTOM | WLR_EDGE_LEFT
                                 | WLR_EDGE_RIGHT);
...
arrange_scratchpad(nnwm_server *server)
{
    ...
    wlr_scene_node_set_position(&server->scene_scratch_dim->node, area.x,
                                area.y);
...
focus_toplevel(nnwm_toplevel *toplevel, bool warp)
{
    ...
    if (prev_surface)
    {
        ...
        {
            /* Check if the previously focused surface belonged to an XWayland
             * window */
            nnwm_toplevel *prev_tl;
            wl_list_for_each(prev_tl, &server->toplevels, link)
            {
                if (prev_tl->is_xwayland
                    && tl_wlr_surface(prev_tl) == prev_surface)
                {
    ...
    wl_list_for_each(tl, &server->toplevels, link)
    {
        ...
        float *color
            = foc ? cfg->border.focused_color : cfg->border.unfocused_color;
#ifdef HAVE_SCENEFX
        ...
        float base
            = (tl->rule_opacity >= 0.0f) ? tl->rule_opacity : cfg->fx.opacity;
        float op
            = foc ? (cfg->fx.focused_opacity >= 0.0f ? cfg->fx.focused_opacity
                                                     : base)
...
static constexpr double SPOT_RADIUS_PX
    = 120.0;                                 /* cutout radius, logical pixels */
...
void
cursor_ring_stop(nnwm_server *server)
{
...
void
cursor_zoom_update_pos(nnwm_server *server)
{
...
/* Upload spotlight buffer at full opacity; scene node opacity handles the
 * envelope. */
static void
spotlight_upload(nnwm_server *server)
{
...
cursor_ring_tick(void *data)
{
    ...
    if (is_spot)
    {
        /* ---- spotlight: GPU opacity envelope + lazy Cairo re-upload on cursor
         * move ---- */

        /* Opacity envelope: fade-in → hold → fade-out (executed on GPU via
         * set_opacity) */
        float opacity;
        if (t < SPOT_HOLD_IN)
    ...
    else if (is_zoom)
    {
        ...
        if (t < ZOOM_HOLD_IN)
        {
            ...
            scale
                = 1.0f + (ZOOM_FACTOR - 1.0f) * std::sin(p * (float)(M_PI / 2));
        }
        ...
        else
        {
            ...
            scale
                = 1.0f + (ZOOM_FACTOR - 1.0f) * std::cos(p * (float)(M_PI / 2));
        }
        server->cursor_zoom_scale = scale;
        int dest_w                = (int)(server->cursor_zoom_img_w * scale);
        int dest_h                = (int)(server->cursor_zoom_img_h * scale);
        int pos_x
            = (int)(server->cursor->x - server->cursor_zoom_hot_x * scale);
        int pos_y
            = (int)(server->cursor->y - server->cursor_zoom_hot_y * scale);
        wlr_scene_buffer_set_dest_size(server->cursor_ring_buf, dest_w, dest_h);
        wlr_scene_node_set_position(&server->cursor_ring_buf->node, pos_x,
                                    pos_y);
    }
    else
    {
        ...
        double dpi = 1.0;
        {
            ...
            wl_list_for_each(out, &server->outputs, link)
            {
                ...
                wlr_output_layout_get_box(server->output_layout,
                                          out->wlr_output, &ob);
                if (server->cursor->x >= ob.x
                    && server->cursor->x < ob.x + ob.width
                    && server->cursor->y >= ob.y
                    && server->cursor->y < ob.y + ob.height)
                {
        ...
        wlr_scene_buffer_set_buffer(server->cursor_ring_buf, &tb->base);
        wlr_buffer_drop(&tb->base);
        wlr_scene_buffer_set_dest_size(server->cursor_ring_buf, RINGS_SIZE_PX,
                                       RINGS_SIZE_PX);
        /* Track live cursor position */
        wlr_scene_node_set_position(
            &server->cursor_ring_buf->node,
...
cursor_ring_start(nnwm_server *server)
{
    ...
    server->cursor_ring_out_scale = 1.0;
    {
        ...
        wl_list_for_each(out, &server->outputs, link)
        {
            ...
            wlr_output_layout_get_box(server->output_layout, out->wlr_output,
                                      &ob);
            if (server->cursor_ring_x >= ob.x
                && server->cursor_ring_x < ob.x + ob.width
                && server->cursor_ring_y >= ob.y
                && server->cursor_ring_y < ob.y + ob.height)
            {
    ...
    if (is_zoom)
    {
        ...
        if (!xc || xc->image_count == 0)
        {
            ...
            xc = wlr_xcursor_manager_get_xcursor(server->cursor_mgr, "default",
                                                 out_scale);
            if (!xc || xc->image_count == 0)
                return;
        ...
        /* Convert straight-alpha xcursor pixels → premultiplied ARGB32 for the
         * scene */
        ...
        for (uint32_t y = 0; y < img->height; y++)
        {
            ...
            uint32_t *drow
                = (uint32_t *)((uint8_t *)tb->data + (size_t)y * tb->stride);
    ...
    else
    {
        ...
        if (is_spotlight)
        {
            ...
            /* Pre-render at full opacity; tick animates via
...
keybind_combo_string(uint32_t mods, xkb_keysym_t keysym)
{
    ...
    if (mods & WLR_MODIFIER_MOD2)
    {
        s += "Mod2+";
    }
    if (mods & WLR_MODIFIER_MOD3)
    {
        s += "Mod3+";
    }
    if (mods & WLR_MODIFIER_MOD5)
    {
        s += "Mod5+";
    }
    char keybuf[64];
    ...
    if (strcmp(keybuf, "Return") == 0)
        s += "Enter";
    else if (strcmp(keybuf, "BackSpace") == 0)
        s += "Backspace";
    else if (strcmp(keybuf, "Prior") == 0)
        s += "PageUp";
    else if (strcmp(keybuf, "Next") == 0)
        s += "PageDown";
    else
        s += keybuf;
    return s;
...
keybind_overlay_render(nnwm_server *server)
{
    ...
    for (int i = 0; i < server->lua_keybinding_count; i++)
    {
        ...
        rows.push_back({keybind_combo_string(kb.mods, kb.keysym),
    ...
    int content_h  = TITLE_H + SEP_H + PAD / 2 + (int)rows.size() * ROW_H + PAD;
    ...
    if (server->keybind_ov_scroll < 0)
        server->keybind_ov_scroll = 0;
    if (server->keybind_ov_scroll > max_scroll)
        server->keybind_ov_scroll = max_scroll;
    ...
    pango_font_description_set_size(fd_title,
                                    (int)((base_sz + 2) * PANGO_SCALE));
    ...
    wlr_fbox src = {0, (double)(server->keybind_ov_scroll * scale), (double)pw,
                    (double)(viewport_h * scale)};
...
keybind_overlay_scroll_by(nnwm_server *server, double delta)
{
    ...
    int max_scroll = std::max(0, server->keybind_ov_content_h
                                     - server->keybind_ov_viewport_h);
    if (server->keybind_ov_scroll < 0)
        server->keybind_ov_scroll = 0;
    if (server->keybind_ov_scroll > max_scroll)
        server->keybind_ov_scroll = max_scroll;
    ...
    float scale  = server->focused_output
                       ? server->focused_output->wlr_output->scale
                       : 1.0f;
