...
static void
item_rebuild_surface(nnwm_tray_item *item, int bar_h);
static void
request_item_properties(nnwm_tray *tray, nnwm_tray_item *item);
static void
parse_item_properties(nnwm_tray *tray, nnwm_tray_item *item,
                      DBusMessage *reply);

/* ---- Icon file search ---- */

static char *
find_icon_png(const char *name, int preferred_size)
{
    if (!name || !name[0])
        return nullptr;
    ...
    if (preferred_size != 22)
        sizes[nsizes++] = 22;
    if (preferred_size != 24)
        sizes[nsizes++] = 24;
    if (preferred_size != 32)
        sizes[nsizes++] = 32;
    if (preferred_size != 16)
        sizes[nsizes++] = 16;
    if (preferred_size != 48)
        sizes[nsizes++] = 48;
    if (preferred_size != 64)
        sizes[nsizes++] = 64;
    if (preferred_size != 128)
        sizes[nsizes++] = 128;
...
scale_surface(cairo_surface_t *src, int target_h)
{
    if (!src)
        return nullptr;
...
static cairo_surface_t *
load_icon_surface_from_file(const char *path, int target_h)
{
    if (!path)
        return nullptr;
...
static cairo_surface_t *
load_icon_surface_from_pixmap(const uint8_t *data, int w, int h, int target_h)
{
    if (!data || w <= 0 || h <= 0)
        return nullptr;
 ...
 */
static bool
parse_pixmap_array(DBusMessageIter *top_iter, int preferred_size,
                   uint8_t **out_data, int *out_w, int *out_h)
{
    ...
    *out_w = *out_h = 0;
    ...
    while (dbus_message_iter_get_arg_type(&arr) == DBUS_TYPE_STRUCT)
    {
        ...
        int nelem          = 0;
        dbus_message_iter_get_fixed_array(&bytes_iter, (const void **)&raw,
                                          &nelem);
...
item_rebuild_surface(nnwm_tray_item *item, int bar_h)
{
    ...
    if (item->icon_name && item->icon_name[0])
    {
        ...
        {
            wlr_log(WLR_INFO, "tray: icon '%s' not found (no PNG)",
                    item->icon_name);
        }
    }
}
...
static void
parse_item_properties(nnwm_tray *tray, nnwm_tray_item *item, DBusMessage *reply)
{
    ...
    wlr_log(WLR_INFO,
            "tray: item '%s' props: status='%s' icon_name='%s' pixmap=%s",
            item->service, item->status ? item->status : "(null)",
...
static void
on_props_reply(DBusPendingCall *pending, void *user_data)
{
...
request_item_properties(nnwm_tray *tray, nnwm_tray_item *item)
{
    DBusMessage *msg = dbus_message_new_method_call(
        item->service, item->object_path, "org.freedesktop.DBus.Properties",
        "GetAll");
    if (!msg)
        return;
...
static nnwm_tray_item *
find_item_by_service(nnwm_tray *tray, const char *service)
{
    nnwm_tray_item *item;
...
static void
register_item(nnwm_tray *tray, const char *service_arg, const char *sender)
{
    ...
    else if (service_arg && strchr(service_arg, '/'))
    {
        ...
        if (service)
        {
            memcpy(service, service_arg, slen);
            service[slen] = 0;
        }
...
static void
unregister_item(nnwm_tray *tray, const char *service)
{
...
static int
watch_dispatch_cb(int /*fd*/, uint32_t mask, void *data)
{
    tray_watch *tw = (tray_watch *)data;
    if (!dbus_watch_get_enabled(tw->watch))
        return 0;
    ...
    if (flags)
        dbus_watch_handle(tw->watch, flags);
...
message_filter(DBusConnection *conn, DBusMessage *msg, void *data)
{
                                    ...
                                    "RegisterStatusNotifierItem"))
    {
        ...
        dbus_message_get_args(msg, &err, DBUS_TYPE_STRING, &service_arg,
                              DBUS_TYPE_INVALID);
    ...
    if (dbus_message_get_type(msg) == DBUS_MESSAGE_TYPE_SIGNAL && iface
        && strcmp(iface, "org.freedesktop.DBus.Properties") == 0 && member
    ...
    if (dbus_message_is_signal(msg, "org.freedesktop.DBus", "NameOwnerChanged"))
    {
        ...
        DBusError err = DBUS_ERROR_INIT;
        dbus_message_get_args(msg, &err, DBUS_TYPE_STRING, &name,
...
tray_init(nnwm_server *server)
{
    ...
    dbus_connection_set_watch_functions(conn, add_watch, remove_watch,
                                        toggle_watch, tray, nullptr);
    dbus_connection_set_timeout_functions(conn, add_timeout, remove_timeout,
                                          toggle_timeout, tray, nullptr);
    ...
    {
        wlr_log(WLR_INFO,
                "tray: could not acquire " SNI_WATCHER_NAME
                " (ret=%d); tray still active as consumer",
                ret);
    }
...
void
tray_destroy(nnwm_server *server)
{
    if (!server->tray)
        return;
...
uint32_t
tray_generation(nnwm_server *server)
{
    if (!server->tray)
        return 0;
...
int
tray_measure_width(nnwm_server *server, int bar_h, int icon_spacing)
{
    if (!server->tray)
        return 0;
    nnwm_tray *tray = (nnwm_tray *)server->tray;
...
int
tray_draw(nnwm_server *server, void *cr_void, int bar_h, int x,
          int icon_spacing)
{
    if (!server->tray)
        return 0;
...
bool
tray_handle_click(nnwm_server *server, double bx, int bar_h, int x_start,
                  int bar_origin_x, int bar_origin_y, uint32_t button)
{
    ...
    wl_list_for_each(item, &tray->items, link)
    {
        ...
        if (bx >= item->rendered_x && bx < item->rendered_x + icon_w)
        {
            ...
            DBusMessage *msg = dbus_message_new_method_call(
                item->service, item->object_path, SNI_ITEM_IFACE, method);
            if (msg)
            {
                dbus_message_append_args(msg, DBUS_TYPE_INT32, &icon_screen_x,
