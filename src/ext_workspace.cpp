...
find_wl_output_for_client(struct wl_client *client,
...
workspace_handle_activate(struct wl_client * /*client*/,
                          struct wl_resource *resource)
{
    auto *ws = static_cast<nnwm_ext_workspace *>(
...
static const struct ext_workspace_handle_v1_interface workspace_impl = {
    workspace_handle_destroy,    workspace_handle_activate,
    workspace_handle_deactivate, workspace_handle_assign,
...
{
    auto *grp = static_cast<nnwm_ext_workspace_group *>(
        wl_resource_get_user_data(resource));
    if (!grp)
        return;
...
{
    auto *mgr = static_cast<nnwm_ext_workspace_manager *>(
        wl_resource_get_user_data(resource));
    if (!mgr)
        return;
    mgr->stopped = true;
                           ...
                           uint32_t version, uint32_t id)
{
    ...
    wl_resource_set_implementation(mgr_res, &manager_impl, mgr,
    ...
    wl_list_for_each(out, &server->outputs, link)
    {
        ...
        wl_resource_set_implementation(grp_res, &group_impl, grp,
        ...
        struct wl_resource *out_res
            = find_wl_output_for_client(client, out->wlr_output);
        if (out_res)
        ...
        for (int i = 0; i < num_ws; i++)
        {
            ...
            ws->resource = ws_res;
            wl_resource_set_implementation(ws_res, &workspace_impl, ws,
...
nnwm::ext_workspace_init(struct nnwm_server *server)
{
    ...
    server->ext_workspace_global = wl_global_create(
        server->wl_display, &ext_workspace_manager_v1_interface, 1, server,
        ...
        wl_list_for_each(grp, &mgr->groups, link)
        {
            ...
            wl_list_for_each(ws, &grp->workspaces, link)
            {
                ...
                if (ws->index >= num_ws)
                {
                     ...
                     * receiving removed, which triggers
                     * workspace_resource_destroy. */
                ...
                else
                {
                    ...
                    if (!ws_name || ws_name[0] == '\0')
                    {
                        snprintf(name_buf, sizeof(name_buf), "%d",
                                 ws->index + 1);
                        ws_name = name_buf;
                    }
            ...
            for (int i = max_live_index + 1; i < num_ws; i++)
            {
                ...
                struct wl_resource *ws_res = wl_resource_create(
                    client, &ext_workspace_handle_v1_interface, (int)version,
                    0);
                if (!ws_res)
                {
                    delete nws;
