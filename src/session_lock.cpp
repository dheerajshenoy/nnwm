...
lock_surface_map(wl_listener *listener, void * /*data*/)
{
    ...
    if (kb)
        wlr_seat_keyboard_notify_enter(
            server->seat, ls->wlr_lock_surface->surface, kb->keycodes,
...
session_lock_new_surface(wl_listener *listener, void *data)
{
    ...
    ls->scene_tree
        = wlr_scene_subsurface_tree_create(server->scene_locks, wls->surface);
    ...
    wl_list_for_each(wls, &lock->wlr_lock->surfaces, link)
    {
        if (wls->data)
            wlr_scene_node_set_enabled(
                &static_cast<nnwm_lock_surface *>(wls->data)->scene_tree->node,
                false);
    }
