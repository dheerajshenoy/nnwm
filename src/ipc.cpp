...
static void
schedule_write(ipc_client *cl)
{
    if (cl->write_source)
        return; /* already armed */
    cl->write_source = wl_event_loop_add_fd(
        wl_display_get_event_loop(cl->server->wl_display), cl->fd,
        WL_EVENT_WRITABLE, [](int /*fd*/, uint32_t /*mask*/, void *data) -> int
    {
...
static void
reply(ipc_client *cl, bool ok, const char *payload)
{
    int n = snprintf(cl->write_buf, sizeof(cl->write_buf), "%s\n", payload);
...
static json
lua_to_json(lua_State *L, int idx, int depth = 0)
{
    if (depth > 32)
...
static bool
exec_lua(lua_State *L, const char *code, char *out, size_t out_size)
{
    char chunk[128 * 1024];
...
static void
handle_client_data(ipc_client *cl)
{
    ssize_t n = read(cl->fd, cl->read_buf + cl->read_len,
                     sizeof(cl->read_buf) - cl->read_len - 1);
    if (n <= 0)
    {
        /* Flush any pending response before closing (client did
         * shutdown(SHUT_WR) so the write source may not have fired yet) */
...
static void
accept_ipc_client(nnwm_server *server)
{
    ...
    cl->read_source = wl_event_loop_add_fd(
        wl_display_get_event_loop(server->wl_display), client_fd,
...
int
ipc_init(nnwm_server *server, struct wl_event_loop *loop)
{
    server->ipc_fd           = -1;
