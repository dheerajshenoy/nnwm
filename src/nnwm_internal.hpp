...
/* Hide or show the bar(s) based on whether any window on the output is
 * fullscreen. Called after fullscreen state changes. */
void bar_update_fullscreen_visibility(struct nnwm_server *server,
                                      struct nnwm_output *out);
