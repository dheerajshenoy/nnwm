...
static inline uint64_t
fnv1a_bytes(uint64_t h, const void *data, size_t n)
{
...
static inline uint64_t
fnv1a_str(uint64_t h, const char *s)
{
    return s ? fnv1a_bytes(h, s, strlen(s)) : h;
}
static inline uint64_t
fnv1a_u32(uint64_t h, uint32_t v)
{
...
static void
tbuf_destroy_impl(struct wlr_buffer *b)
{
...
static bool
tbuf_data_ptr(struct wlr_buffer *b, uint32_t, void **data, uint32_t *format,
              size_t *stride)
{
    bar_tbuf *tb = wl_container_of(b, tb, base);
    *data        = tb->data;
...
static void
tbuf_data_ptr_end(struct wlr_buffer *)
{
}
...
static const char *
layout_mode_name(nnwm_layout_mode m)
{
    switch (m)
    {
...
static nnwm_output *
bar_target_output(nnwm_bar *bar)
{
    if (bar->output)
        return bar->output;
...
bar_focused_toplevel(nnwm_bar *bar)
{
...
static int
draw_text_segment(cairo_t *cr, PangoFontDescription *fd, const char *text,
                  int x, int bar_h, const float fg[4], const float bg[4],
                  int pad)
{
    if (!text || !text[0])
        return 0;
 ...
 * was modified. */
static bool
cache_set(nnwm_bar_module &m, const char *s)
{
...
static void
module_refresh(nnwm_bar *bar, nnwm_bar_module &m, double /*now*/)
{
    switch (m.type)
    {
        ...
        case nnwm_bar_module_type::LAYOUT:
        {
            ...
            cache_set(
                m, layout_mode_name(out->layout_mode[out->active_workspace]));
            return;
        }
...
static void
module_tick_poll(nnwm_bar *bar, nnwm_bar_module &m, double now)
{
    ...
    {
        wlr_log(WLR_ERROR, "bar custom widget error: %s", lua_tostring(L, -1));
    }
    cache_set(m, result);
    m.cached_ts = now;
    lua_pop(L, 1);
...
static const float *
pick(const float mod[4], const float fallback[4])
{
...
static uint16_t
workspace_occupancy_bits(nnwm_server *server, nnwm_output *out)
{
    if (!out)
        return 0;
    uint16_t bits = 0;
...
static int
draw_workspaces_module(cairo_t *cr, PangoFontDescription *fd, nnwm_bar *bar,
                       nnwm_bar_module &m, int x, int bar_h, uint16_t occ_bits,
                       int active_ws, const int *pill_widths = nullptr,
                       int pill_height = 0)
{
    nnwm_config *cfg = bar->server->config;
    nnwm_output *out = bar_target_output(bar);
    int count        = cfg->workspace_count > 0 ? cfg->workspace_count : 9;
    if (count > NNWM_NUM_WORKSPACES)
        count = NNWM_NUM_WORKSPACES;
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (out && out->workspace_names[i])
            label = out->workspace_names[i];
        else if (cfg->workspace_names[i])
            label = cfg->workspace_names[i];
        char buf[16];
        if (!label)
        {
        ...
        {
            used += draw_text_segment(cr, fd, label, x + used, bar_h, fg, bg,
                                      pad);
        }
    }
    if (shared)
...
static void
bar_refresh_modules(nnwm_bar *bar, double now)
{
    nnwm_config *cfg = bar->server->config;
...
static uint64_t
bar_hash(nnwm_bar *bar, uint16_t occ_bits, int active_ws)
{
    nnwm_config *cfg = bar->server->config;
...
bar_redraw(nnwm_bar *bar)
{
    ...
    if (bar->module_type_mask
        & (1u << (unsigned)nnwm_bar_module_type::WORKSPACES))
        occ_bits = workspace_occupancy_bits(server, out);
    uint64_t hash = bar_hash(bar, occ_bits, active_ws);
    if (bar->has_last_hash && bar->last_hash == hash)
        return;
    bar->last_hash     = hash;
    ...
    {
        if (i < bar->module_font_descs_count && bar->module_font_descs[i])
    ...
    for (int i = 0; i < cfg->bar.module_count; i++)
    {
        ...
        if (m.type == nnwm_bar_module_type::TRAY)
        {
            ...
            segs[i].w = tray_measure_width(bar->server, bar->height, spacing)
                        + 2 * pad;
        }
        else if (m.type == nnwm_bar_module_type::WORKSPACES)
        {
            ...
            {
                const char *label = nullptr;
                if (tout && tout->workspace_names[j])
                    label = tout->workspace_names[j];
                else if (cfg->workspace_names[j])
                    label = cfg->workspace_names[j];
                char buf[16];
                if (!label)
                {
                ...
                pango_layout_set_text(shared, label, -1);
                int tw, th;
                pango_layout_get_pixel_size(shared, &tw, &th);
                pw_arr[j] = tw;
                if (th > pill_h)
                    pill_h = th;
                w += tw + 2 * pad;
        ...
        else
        {
            ...
            pango_layout_get_pixel_size(segs[i].layout, &segs[i].text_w,
                                        &segs[i].text_h);
    ...
    for (int i = 0; i < cfg->bar.module_count; i++)
    {
        ...
        if (s.m->type == nnwm_bar_module_type::TRAY)
        {
            ...
            int icon_w
                = tray_draw(bar->server, cr, bar->height, x + pad, spacing);
            drawn = icon_w + 2 * pad;
        }
        else if (s.is_ws)
        {
            drawn = draw_workspaces_module(cr, module_font(i), bar, *s.m, x,
                                           bar->height, occ_bits, active_ws,
                 ...
                 && s.m->align == nnwm_bar_align::CENTER)
        {
            ...
            if (s.w > max_w)
            {
                ...
                pango_layout_set_width(s.layout,
                                       (max_w - 2 * pad) * PANGO_SCALE);
        ...
        if (s.m->align == nnwm_bar_align::LEFT)
            left_cursor = cursor[idx];
    }
    for (int i = 0; i < cfg->bar.module_count; i++)
...
bar_layout(nnwm_bar *bar)
{
    ...
    bar->x            = ob.x + ml;
    bar->y = cfg->bar.position_top ? ob.y + mt
    ...
    if (bar->fx_shadow)
    {
        ...
        float sc[4]
            = {cfg->bar.fx.shadow_color[0], cfg->bar.fx.shadow_color[1],
               cfg->bar.fx.shadow_color[2], cfg->bar.fx.shadow_color[3] * o};
...
static int
bar_tick_interval_ms(nnwm_config *cfg)
{
    int best = 0;
    for (int i = 0; i < cfg->bar.module_count; i++)
    {
        nnwm_bar_module &m = cfg->bar.modules[i];
        int cand           = 0;
        if (m.type == nnwm_bar_module_type::CLOCK)
            cand = 1000;
...
static nnwm_bar *
bar_create(nnwm_server *server, nnwm_output *out)
{
    nnwm_config *cfg = server->config;
    if (cfg->bar.height <= 0)
        return nullptr;
    ...
    {
        bar->module_font_descs
            = static_cast<struct _PangoFontDescription **>(calloc(
                cfg->bar.module_count, sizeof(struct _PangoFontDescription *)));
        ...
        for (int i = 0; i < cfg->bar.module_count; i++)
        {
            ...
            if (m.font && m.font[0])
            {
                ...
                if (!fam || !fam[0])
                {
                    ...
                    pango_font_description_set_family(
                        fd, pango_font_description_get_family(base));
                }
            }
            else
            {
            ...
            if (m.style && m.style[0])
            {
                ...
                if (!strcmp(m.style, "italic"))
                    s = PANGO_STYLE_ITALIC;
                else if (!strcmp(m.style, "oblique"))
            ...
            if (m.weight && m.weight[0])
            {
                ...
                if (!strcmp(m.weight, "thin"))
                    w = PANGO_WEIGHT_THIN;
                else if (!strcmp(m.weight, "ultralight"))
                    w = PANGO_WEIGHT_ULTRALIGHT;
                else if (!strcmp(m.weight, "light"))
                    w = PANGO_WEIGHT_LIGHT;
                else if (!strcmp(m.weight, "normal"))
                    w = PANGO_WEIGHT_NORMAL;
                else if (!strcmp(m.weight, "medium"))
                    w = PANGO_WEIGHT_MEDIUM;
                else if (!strcmp(m.weight, "semibold"))
                    w = PANGO_WEIGHT_SEMIBOLD;
                else if (!strcmp(m.weight, "bold"))
                    w = PANGO_WEIGHT_BOLD;
                else if (!strcmp(m.weight, "ultrabold"))
                    w = PANGO_WEIGHT_ULTRABOLD;
                else if (!strcmp(m.weight, "heavy"))
...
static void
bar_destroy(nnwm_bar *bar)
{
    if (!bar)
        return;
    if (bar->tick_timer)
        wl_event_source_remove(bar->tick_timer);
    if (bar->tree)
...
void
bar_apply_config(nnwm_server *server)
{
    nnwm_config *cfg = server->config;
    ...
    if (!cfg->bar.enabled || cfg->bar.height <= 0)
        return;
    ...
    else
    {
        ...
        if (target)
            server->global_bar = bar_create(server, target);
    }
}

/* Subtract bar height from usable_area if a bar occupies this output. */
void
bar_shrink_usable_area(nnwm_server *server, nnwm_output *out, wlr_box *usable)
{
    nnwm_config *cfg = server->config;
    if (!cfg->bar.enabled || cfg->bar.height <= 0)
        return;
    ...
    bool has_bar = false;
    if (cfg->bar.per_output)
        has_bar = (out->bar != nullptr);
    else
        has_bar = (server->global_bar
...
static constexpr unsigned MASK_WORKSPACES
    = 1u << (unsigned)nnwm_bar_module_type::WORKSPACES;
static constexpr unsigned MASK_WINDOW_TITLE
    = 1u << (unsigned)nnwm_bar_module_type::WINDOW_TITLE;
static constexpr unsigned MASK_LAYOUT
    = 1u << (unsigned)nnwm_bar_module_type::LAYOUT;

static inline bool
bar_cares_about(nnwm_bar *bar, unsigned mask)
{
...
static void
redraw_if_cares(nnwm_bar *bar, unsigned mask)
{
    if (!bar_cares_about(bar, mask))
        return;
    bar->dirty       = true;
...
void
bar_notify_workspace_change(nnwm_server *server, nnwm_output *out)
{
...
void
bar_notify_focus_change(nnwm_server *server)
{
...
void
bar_notify_windows_changed(nnwm_server *server)
{
...
void
bar_destroy_all(nnwm_server *server)
{
    bar_destroy(server->global_bar);
    server->global_bar = nullptr;
    nnwm_output *out;
...
void
bar_predraw_output(nnwm_server *server, nnwm_output *out)
{
    if (out->bar && out->bar->dirty)
        bar_redraw(out->bar);
...
void
bar_destroy_for_output(nnwm_output *out)
{
    if (!out)
        return;
    bar_destroy(out->bar);
...
void
bar_update_fullscreen_visibility(nnwm_server *server, nnwm_output *out)
{
    if (!out)
        return;
    nnwm_config *cfg = server->config;
...
static nnwm_bar *
bar_at(nnwm_server *server, double lx, double ly, double *local_x,
       double *local_y)
{
    auto hit = [&](nnwm_bar *bar) -> bool
    {
        if (!bar || !bar->tree || !bar->tree->node.enabled)
            return false;
        if (lx < bar->x || lx >= bar->x + bar->width)
            return false;
        if (ly < bar->y || ly >= bar->y + bar->height)
            return false;
        *local_x = lx - bar->x;
        *local_y = ly - bar->y;
        return true;
    };
    if (hit(server->global_bar))
        return server->global_bar;
    nnwm_output *out;
    wl_list_for_each(out, &server->outputs,
                     link) if (hit(out->bar)) return out->bar;
    return nullptr;
}
...
static int
bar_module_at(nnwm_bar *bar, double bx, double by)
{
    nnwm_config *cfg = bar->server->config;
    for (int i = 0; i < cfg->bar.module_count; i++)
    {
        ...
        if (m.rect_w <= 0)
            continue;
        if (bx >= m.rect_x && bx < m.rect_x + m.rect_w && by >= m.rect_y
            && by < m.rect_y + m.rect_h)
            return i;
    }
    return -1;
}
...
static void
fire_hover(nnwm_server *server, int ref, bool entered)
{
    if (ref < 0 || !server->lua)
        return;
    lua_State *L = server->lua;
...
bool
bar_handle_motion(nnwm_server *server, double lx, double ly)
{
    double bx, by;
    ...
    if (!bar)
    {
        auto leave = [&](nnwm_bar *b)
        {
            if (!b)
                return;
            if (b->hover_module_idx == -2)
                return;
            if (b->hover_module_idx >= 0
                && b->hover_module_idx < cfg->bar.module_count)
            {
                fire_hover(server,
                           cfg->bar.modules[b->hover_module_idx].lua_hover_ref,
                           false);
            }
    ...
    if (idx == bar->hover_module_idx)
        return true; /* still consuming events */
    ...
    {
        fire_hover(server,
                   cfg->bar.modules[bar->hover_module_idx].lua_hover_ref,
                   false);
    }
...
bool
bar_handle_button(nnwm_server *server, double lx, double ly, uint32_t button,
                  bool pressed)
{
    double bx, by;
    nnwm_bar *bar = bar_at(server, lx, ly, &bx, &by);
    ...
    if (idx >= 0 && cfg->bar.modules[idx].type == nnwm_bar_module_type::TRAY)
    {
        ...
        tray_handle_click(server, bx, bar->height, m.rect_x, bar->x, bar->y,
                          button);
        return true;
    }
    ...
    int click_ref = (idx >= 0) ? cfg->bar.modules[idx].lua_click_ref
    ...
    if (idx >= 0 && button == BTN_LEFT)
    {
        ...
        if (m.type == nnwm_bar_module_type::WORKSPACES && out)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (cfg->bar.smart_workspaces && i != out->active_workspace
                    && !is_occ)
                    continue;

                const char *label = nullptr;
                if (out->workspace_names[i])
                    label = out->workspace_names[i];
                else if (cfg->workspace_names[i])
                    label = cfg->workspace_names[i];
                char buf[16];
...
void
bar_notify_tray_changed(nnwm_server *server)
{
    static constexpr unsigned MASK_TRAY
        = 1u << (unsigned)nnwm_bar_module_type::TRAY;
    nnwm_output *out;
    wl_list_for_each(out, &server->outputs, link)
...
void
bar_update_module(nnwm_server *server, const char *name)
{
    if (!name || !name[0])
        return;
    nnwm_config *cfg = server->config;
    ...
    for (int i = 0; i < cfg->bar.module_count; i++)
    {
        ...
        if (m.name && strcmp(m.name, name) == 0)
        {
            ...
            if (any_bar)
                module_tick_poll(any_bar, m, now);
            any = true;
        }
    }
    if (!any)
