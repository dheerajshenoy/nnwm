...
extern "C"
{
    ...
    /* clipped_region.h only uses wayland/wlroots primitives already included
     * above */
    ...
    struct wlr_scene_shadow *
    wlr_scene_shadow_create(struct wlr_scene_tree *parent, int width,
                            int height, int corner_radius, float blur_sigma,
                            const float color[4]);
    void wlr_scene_shadow_set_size(struct wlr_scene_shadow *shadow, int width,
                                   int height);
    ...
    void wlr_scene_blur_set_size(struct wlr_scene_blur *blur, int width,
                                 int height);
...
struct nnwm_server
{
    ...
    struct nnwm_session_lock
        *session_lock; /* non-null while screen is locked */
    struct wlr_scene_tree
        *scene_locks; /* above overlay — lock surfaces live here */
    ...
    struct wl_list
        ext_workspace_managers; /* nnwm_ext_workspace_manager::link */
    ...
    struct wl_event_source
        *error_dismiss_timer; /* auto-hides config error bar */
...
struct nnwm_toplevel
{
    ...
    struct nnwm_server *server;
    struct nnwm_output
        *output;   /* owning output — which monitor this window lives on */
    ...
    struct nnwm_decoration
        *decoration; /* pending decoration, applied on initial commit */
    struct wlr_scene_buffer
        *titlebar;      /* server-side titlebar, nullptr if disabled */
    int titlebar_width; /* last rendered inner width (for title-change redraws)
                         */
#ifdef HAVE_SCENEFX
    struct wlr_scene_shadow
        *fx_shadow; /* scenefx shadow node, nullptr if disabled */
    struct wlr_scene_blur
        *fx_blur; /* scenefx background blur node, nullptr if disabled */
#endif
...
struct nnwm_popup
{
    ...
    struct wlr_scene_tree *parent_tree; /* immediate parent surface scene tree
                                           (for scene parenting) */
    struct wlr_scene_tree
        *root_tree; /* root surface scene tree (for constraint coords) */
    struct wlr_scene_tree *offset_tree; /* intermediate tree at output origin
                                           for null-parent popups */
    struct wl_listener commit;
    struct wl_listener destroy;
};
